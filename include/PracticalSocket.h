...
/**
 *   TCP socket class for servers that does not block
 */
class NonBlockTCPServerSocket : public TCPServerSocket {
public:
  /**
   *   Construct a TCP socket for use with a server, accepting connections
   *   on the specified port on any interface, but don't block on accept
   *   @param localPort local port of server socket, a value of zero will
   *                   give a system-assigned unused port
   *   @param queueLen maximum queue length for outstanding 
   *                   connection requests (default 5)
   *   @exception SocketException thrown if unable to create TCP server socket
   */
  NonBlockTCPServerSocket(unsigned short localPort, int queueLen = 5) 
      throw(SocketException);

private:
  void setNonBlock() throw(SocketException);
};
