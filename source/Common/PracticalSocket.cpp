...
// NonBlockTCPServerSocket
NonBlockTCPServerSocket::NonBlockTCPServerSocket(unsigned short localPort, int queueLen)
	throw(SocketException): TCPServerSocket(localPort, queueLen)
{
	setNonBlock();
}

#include <stdio.h>
void NonBlockTCPServerSocket::setNonBlock() throw(SocketException)
{
	int x = fcntl(sockDesc, F_GETFL, 0);
	int rc = fcntl(sockDesc, F_SETFL, x | O_NONBLOCK);
